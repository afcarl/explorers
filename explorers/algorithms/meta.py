...
defcfg._branch('ex_1') # first explorer
#defcfg._branch('ex_2') # second explorer
...
class MetaExplorer(explorer.Explorer):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        for i, _ in enumerate(self.cfg.weights[0]):
            ...
            self.explorers.append(explorers.Explorer.create(ex_cfg, **kwargs))

    def explore(self):
        if (self.timecount >= self.cfg.eras[self.current_era]):
            self.current_era += 1

        idx = explorers.tools(self.cfg.weights[self.current_era])
